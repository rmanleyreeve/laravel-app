...
Route::get ('/dashboard', function(Request $request) {
	...
	if(!$res) {
		...
		return view('dashboard', ['selected' => reset($res)]);
	}
})->name('home');
...
Route::post('/report', function (Request $request) {
	...
	if($res) {
		...
		return view('report-issue', ['reported' => true]);
	}
});
...
Route::get ('/inspections/{id}/signature', function($id) {
	if (!isset($_COOKIE['tenant_id'])) {
		return redirect()->route('default');
	}
	$res = DB::select(
...
Route::post ('/update', function(Request $request) {
	...
	$dataObj->update($vars);
	$dataObj->save();
	return view('update-details', ['updated' => true]);
});
